...
const NavBar2 = () => {
    ...
    return (
        <div ref={ref} className="z-50 fixed top-0 left-0 right-0 bg-amber-400">
            <nav className={`fixed top-0 left-0 bg-indigo-500 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
                ...
                {/* Mobile Menu */}
                <div className={`fixed top-0 left-0 w-full h-screen bg-white/60 backdrop-blur-2xl text-base flex flex-col md:hidden items-center justify-center gap-6 font-medium text-gray-800 transition-all duration-500 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
